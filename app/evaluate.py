...
def add_entry_to_past_evaluations(perplexity, model, dataset, stride, max_length):
    ...
    past_evaluations = pd.concat(
        [past_evaluations, pd.DataFrame([entry])], ignore_index=True
    )


def is_in_past_evaluations(model, dataset, stride, max_length):
    entries = past_evaluations[
        (past_evaluations["Model"] == model)
...
def generate_markdown_table():
    sorted_df = past_evaluations.sort_values(
