...
from app import presets, shared
...
from app.settings import apply_model_settings_to_state, get_model_settings, save_model_settings
    ...
    try:
        ...
        if check:
            ...
            downloader.download_model_files(
                model, branch, links, sha256, output_folder, progress_bar=progress, threads=1
            )
            yield ("Done!")
    except:
        progress(1.0)
...
def create_model_menus():
            ...
            with gr.Row():
                ...
                with gr.Column():
                    with gr.Row():
                        ...
                        shared.gradio["lora_menu_apply"] = gr.Button(
                            value="Apply LoRAs", elem_classes="refresh-button"
                        )

    with gr.Row():
        with gr.Column():
            shared.gradio["loader"] = gr.Dropdown(
                label="Model loader",
                choices=[
                    "Transformers",
                    "ExLlama_HF",
            ...
            with gr.Box():
                with gr.Row():
                    with gr.Column():
                        for i in range(len(total_mem)):
                            shared.gradio[f"gpu_memory_{i}"] = gr.Slider(
                                label=f"gpu-memory in MiB for device :{i}",
                                maximum=total_mem[i],
                                value=default_gpu_mem[i],
                            )

                        shared.gradio["cpu_memory"] = gr.Slider(
                            label="cpu-memory in MiB", maximum=total_cpu_mem, value=default_cpu_mem
                        )
                        ...
                        shared.gradio["quant_type"] = gr.Dropdown(
                            label="quant_type", choices=["nf4", "fp4"], value=shared.args.quant_type
                        )
                        ...
                        shared.gradio["n_batch"] = gr.Slider(
                            label="n_batch", minimum=1, maximum=2048, value=shared.args.n_batch
                        )
                        ...
                        shared.gradio["wbits"] = gr.Dropdown(
                            ...
                            value=str(shared.args.wbits) if shared.args.wbits > 0 else "None",
                        )
                        shared.gradio["groupsize"] = gr.Dropdown(
                            label="groupsize",
                            choices=["None", 32, 64, 128, 1024],
                            value=str(shared.args.groupsize)
                        ...
                        shared.gradio["autogptq_info"] = gr.Markdown(
                            "* ExLlama_HF is recommended over AutoGPTQ for models derived from LLaMA."
                        )
                        shared.gradio["gpu_split"] = gr.Textbox(
                            label="gpu-split",
                            info="Comma-separated list of VRAM (in GB) to use per GPU. Example: 20,7,7",
                        )
                        shared.gradio["max_seq_len"] = gr.Slider(
                            label="max_seq_len",
                            minimum=2048,
                            maximum=16384,
                            step=256,
                        ...
                        shared.gradio["compress_pos_emb"] = gr.Slider(
                            label="compress_pos_emb",
                            minimum=1,
                            maximum=8,
                            step=1,
                        ...
                        shared.gradio["alpha_value"] = gr.Slider(
                            label="alpha_value",
                            minimum=1,
                            maximum=32,
                            step=1,
                            info="Positional embeddings alpha factor for NTK RoPE scaling. Scaling is not identical to embedding compression. Use either this or compress_pos_emb, not both.",
                            value=shared.args.alpha_value,
                        )
                    ...
                    with gr.Column():
                        ...
                        shared.gradio["no_inject_fused_mlp"] = gr.Checkbox(
                            label="no_inject_fused_mlp",
                        ...
                        shared.gradio["no_use_cuda_fp16"] = gr.Checkbox(
                            label="no_use_cuda_fp16",
                        ...
                        shared.gradio["low_vram"] = gr.Checkbox(
                            label="low-vram", value=shared.args.low_vram
                        )
                        shared.gradio["mlock"] = gr.Checkbox(label="mlock", value=shared.args.mlock)
                        shared.gradio["llama_cpp_seed"] = gr.Number(
                            label="Seed (0 for random)", value=shared.args.llama_cpp_seed
                        )
                        ...
                        shared.gradio["llamacpp_HF_info"] = gr.Markdown(
                            'llamacpp_HF is a wrapper that lets you use llama.cpp like a Transformers model, which means it can use the Transformers samplers. It works, but it\'s experimental and slow. Contributions are welcome.\n\nTo use it, make sure to first download oobabooga/llama-tokenizer under "Download custom model or LoRA".'
                        )
        ...
        with gr.Column():
            ...
            shared.gradio["custom_model_menu"] = gr.Textbox(
                label="Download custom model or LoRA",
    ...
    shared.gradio["loader"].change(
        params.make_loader_params_visible, gradio("loader"), gradio(params.get_all_params())
    )
    ...
    shared.gradio["model_menu"].change(
        ...
        load_model_wrapper,
        gradio("model_menu", "loader", "autoload_model"),
        gradio("model_status"),
        show_progress=False,
    )
    ...
    load.click(
        ui.gather_interface_values, gradio(shared.input_elements), gradio("interface_state")
    ).then(update_model_parameters, gradio("interface_state"), None).then(
        partial(load_model_wrapper, autoload=True),
        gradio("model_menu", "loader"),
        gradio("model_status"),
        show_progress=False,
    )
    ...
    shared.gradio["lora_menu_apply"].click(
        load_lora_wrapper, gradio("lora_menu"), gradio("model_status"), show_progress=False
    )
    shared.gradio["download_model_button"].click(
        download_model_wrapper,
        gradio("custom_model_menu"),
        gradio("model_status"),
        show_progress=True,
    )
    shared.gradio["autoload_model"].change(
        lambda x: gr.update(visible=not x), gradio("autoload_model"), load
    )
        ...
        with gr.Row():
            ...
            with gr.Column():
                shared.gradio["stop_at_newline"] = gr.Checkbox(
                    value=shared.settings["stop_at_newline"],
            ...
            with gr.Box():
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        shared.gradio["repetition_penalty_range"] = gr.Slider(
                            0,
                            4096,
                            step=64,
                        ...
                        shared.gradio["min_length"] = gr.Slider(
                            0, 2000, step=1, value=generate_params["min_length"], label="min_length"
                        )
                        shared.gradio["tfs"] = gr.Slider(
                            0.0, 1.0, value=generate_params["tfs"], step=0.01, label="tfs"
                        )
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        shared.gradio["penalty_alpha"] = gr.Slider(
                            0, 5, value=generate_params["penalty_alpha"], label="penalty_alpha"
                        )

                        gr.Markdown("Beam search")
                        shared.gradio["num_beams"] = gr.Slider(
                            1, 20, step=1, value=generate_params["num_beams"], label="num_beams"
                        )
                        shared.gradio["length_penalty"] = gr.Slider(
                            -5, 5, value=generate_params["length_penalty"], label="length_penalty"
                        )
                        shared.gradio["early_stopping"] = gr.Checkbox(
                            value=generate_params["early_stopping"], label="early_stopping"
                        )
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        shared.gradio["stream"] = gr.Checkbox(
                            value=not shared.args.no_stream, label="Activate text streaming"
                        )
...
def create_file_saving_menus():
    ...
    with gr.Box(visible=False, elem_classes="file-saver") as shared.gradio["file_saver"]:
        ...
        shared.gradio["save_root"] = gr.Textbox(
            lines=1, label="File folder", info="For reference. Unchangeable.", interactive=False
        )
    ...
    with gr.Box(visible=False, elem_classes="file-saver") as shared.gradio["file_deleter"]:
        ...
        with gr.Row():
            shared.gradio["delete_confirm"] = gr.Button(
                "Delete", elem_classes="small-button", variant="stop"
            )
    ...
    if shared.is_chat():
        with gr.Box(visible=False, elem_classes="file-saver") as shared.gradio["character_saver"]:
            shared.gradio["save_character_filename"] = gr.Textbox(
                lines=1,
                label="File name",
                info="The character will be saved to your characters/ folder with this base filename.",
            )
            with gr.Row():
                shared.gradio["save_character_confirm"] = gr.Button(
                    "Save", elem_classes="small-button"
                )
                shared.gradio["save_character_cancel"] = gr.Button(
        ...
        with gr.Box(visible=False, elem_classes="file-saver") as shared.gradio["character_deleter"]:
            ...
            with gr.Row():
                shared.gradio["delete_character_confirm"] = gr.Button(
                    "Delete", elem_classes="small-button", variant="stop"
                )
                shared.gradio["delete_character_cancel"] = gr.Button(
                    "Cancel", elem_classes="small-button"
                )
...
def create_file_saving_event_handlers():
    shared.gradio["save_confirm"].click(
        ...
        None,
    ).then(lambda: gr.update(visible=False), None, gradio("file_saver"))

    shared.gradio["delete_confirm"].click(
        lambda x, y: utils.delete_file(x + y), gradio("delete_root", "delete_filename"), None
    ).then(lambda: gr.update(visible=False), None, gradio("file_deleter"))

    shared.gradio["delete_cancel"].click(
    ...
    if shared.is_chat():
        shared.gradio["save_character_confirm"].click(
            chat.save_character,
            gradio("name2", "greeting", "context", "character_picture", "save_character_filename"),
            None,
        ...
        ).then(lambda: gr.update(visible=False), None, gradio("character_deleter")).then(
            lambda: gr.update(choices=utils.get_available_characters()),
            None,
            gradio("character_menu"),
        )

        shared.gradio["save_character_cancel"].click(
            lambda: gr.update(visible=False), None, gradio("character_saver")
        )
        shared.gradio["delete_character_cancel"].click(
            lambda: gr.update(visible=False), None, gradio("character_deleter")
        )

    shared.gradio["save_preset"].click(
        ui.gather_interface_values, gradio(shared.input_elements), gradio("interface_state")
    ).then(presets.generate_preset_yaml, gradio("interface_state"), gradio("save_contents")).then(
        lambda: "presets/", None, gradio("save_root")
    ).then(
        lambda: "My Preset.yaml", None, gradio("save_filename")
    ).then(
        lambda: gr.update(visible=True), None, gradio("file_saver")
    )

    shared.gradio["delete_preset"].click(
        lambda x: f"{x}.yaml", gradio("preset_menu"), gradio("delete_filename")
    ).then(lambda: "presets/", None, gradio("delete_root")).then(
    ...
    if not shared.args.multi_user:
        ...
        def load_session(session, state):
            ...
            if shared.is_chat():
                chat.save_persistent_history(
                    state["history"], state["character_menu"], state["mode"]
                )
        ...
        if shared.is_chat():
            shared.gradio["save_session"].click(
                ...
                lambda: "logs/", None, gradio("save_root")
            ).then(
                lambda x: f'session_{shared.get_mode()}_{x + "_" if x not in ["None", None, ""] else ""}{utils.current_time()}.json',
                gradio("character_menu"),
            ...
            shared.gradio["session_menu"].change(
                load_session, gradio("session_menu", "interface_state"), gradio("interface_state")
            ).then(
                ui.apply_interface_values,
                gradio("interface_state"),
                gradio(ui.list_interface_input_elements()),
            ...
            shared.gradio["save_session"].click(
                ...
                lambda: f"session_{shared.get_mode()}_{utils.current_time()}.json",
                None,
                gradio("save_filename"),
            ).then(
                lambda: gr.update(visible=True), None, gradio("file_saver")
...
def create_interface():
    ...
    with gr.Blocks(css=css, analytics_enabled=False, title=title, theme=ui.theme) as shared.gradio[
        "interface"
    ]:
            ...
            with gr.Tab("Text generation", elem_id="main"):
                ...
                with gr.Row():
                    ...
                    shared.gradio["Clear history-confirm"] = gr.Button(
                        "Confirm", variant="stop", visible=False
                    )
            ...
            with gr.Tab("Chat settings", elem_id="chat-settings"):
                    ...
                    with gr.Row():
                        with gr.Column(scale=8):
                            with gr.Row():
                                ...
                                shared.gradio["save_character"] = gr.Button(
                                    "💾", elem_classes="refresh-button"
                                )
                                shared.gradio["delete_character"] = gr.Button(
                                    "🗑️", elem_classes="refresh-button"
                                )

                            shared.gradio["name1"] = gr.Textbox(
                                value=shared.settings["name1"], lines=1, label="Your name"
                            )
                            shared.gradio["name2"] = gr.Textbox(
                                value=shared.settings["name2"], lines=1, label="Character's name"
                            )
                            shared.gradio["context"] = gr.Textbox(
                                value=shared.settings["context"], lines=4, label="Context"
                            )
                            shared.gradio["greeting"] = gr.Textbox(
                                value=shared.settings["greeting"], lines=4, label="Greeting"
                            )

                        with gr.Column(scale=1):
                            shared.gradio["character_picture"] = gr.Image(
                                label="Character picture", type="pil"
                            )
                ...
                with gr.Tab("Instruction template"):
                    with gr.Row():
                        with gr.Row():
                            ...
                            shared.gradio["save_template"] = gr.Button(
                                "💾", elem_classes="refresh-button"
                            )
                            shared.gradio["delete_template"] = gr.Button(
                                "🗑️ ", elem_classes="refresh-button"
                            )

                    shared.gradio["name1_instruct"] = gr.Textbox(
                        value="", lines=2, label="User string"
                    )
                    shared.gradio["name2_instruct"] = gr.Textbox(
                        value="", lines=1, label="Bot string"
                    )
                    shared.gradio["context_instruct"] = gr.Textbox(
                        value="", lines=4, label="Context"
                    )
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            shared.gradio["upload_chat_history"] = gr.File(
                                type="binary", file_types=[".json", ".txt"], label="Upload"
                            )
                ...
                with gr.Tab("Upload character"):
                    with gr.Tab("JSON"):
                        ...
                        shared.gradio["Submit character"] = gr.Button(
                            value="Submit", interactive=False
                        )

                    with gr.Tab("TavernAI"):
                        with gr.Row():
                            with gr.Column():
                                shared.gradio["upload_img_tavern"] = gr.Image(
                                    type="pil",
                        ...
                        shared.gradio["Submit tavern character"] = gr.Button(
                            value="Submit", interactive=False
                        )
                    ...
                    with gr.Column(scale=1):
                        ...
                        with gr.Row():
                            ...
                            shared.gradio["delete_prompt"] = gr.Button(
                                "🗑️", elem_classes=["refresh-button", "refresh-button-small"]
                            )
                    ...
                    with gr.Column():
                        with gr.Tab("Raw"):
                            shared.gradio["output_textbox"] = gr.Textbox(
                                elem_classes="textbox_default_output", lines=27, label="Output"
                            )
        ...
        with gr.Tab("Session", elem_id="session-tab"):
            ...
            bool_list = sorted(
                [
                    k
                    for k in cmd_list
                    if type(cmd_list[k]) is bool and k not in modes + ui.list_model_elements()
                ]
            )
            ...
            with gr.Row():
                with gr.Column():
                    with gr.Row():
                        ...
                        shared.gradio["reset_interface"] = gr.Button(
                            "Apply and restart", elem_classes="small-button", variant="primary"
                        )
                        shared.gradio["toggle_dark_mode"] = gr.Button(
                            "Toggle 💡", elem_classes="small-button"
                        )
                    ...
                    with gr.Row():
                        with gr.Column():
                            shared.gradio["extensions_menu"] = gr.CheckboxGroup(
                                ...
                                elem_classes="checkboxgroup-table",
                            )
                ...
                with gr.Column():
                    if not shared.args.multi_user:
                        with gr.Row():
                            shared.gradio["session_menu"] = gr.Dropdown(
                                choices=utils.get_available_sessions(),
                                value="None",
                                label="Session",
                                elem_classes="slim-dropdown",
                                info="When saving a session, make sure to keep the initial part of the filename (session_chat, session_notebook, or session_default), otherwise it will not appear on this list afterwards.",
                            )
                            ui.create_refresh_button(
                            ...
                            shared.gradio["delete_session"] = gr.Button(
                                "🗑️", elem_classes=["refresh-button"]
                            )

                    extension_name = gr.Textbox(
                        lines=1,
                        label="Install or update an extension",
                        info="Enter the GitHub URL below and press Enter. For a list of extensions, see: https://github.com/oobabooga/text-generation-webui-extensions ⚠️  WARNING ⚠️ : extensions can execute arbitrary code. Make sure to inspect their source code before activating them.",
                    )
                    extension_status = gr.Markdown()

            extension_name.submit(
                clone_or_pull_repository, extension_name, extension_status, show_progress=False
            ).then(
                lambda: gr.update(
                    choices=utils.get_available_extensions(), value=shared.args.extensions
                ),
                None,
            ...
            shared.gradio["reset_interface"].click(
                ...
                None,
                None,
                _js="() => {document.body.innerHTML='<h1 style=\"font-family:monospace;padding-top:20%;margin:0;height:100vh;color:lightgray;text-align:center;background:var(--body-background-fill)\">Reloading...</h1>'; setTimeout(function(){location.reload()},2500); return []}",
            )
        ...
        if shared.is_chat():
            ...
            gen_events.append(
                ...
                .then(lambda: None, None, None, _js=f"() => {{{audio_notification_js}}}")
            )
            ...
            gen_events.append(
                ...
                .then(lambda x: x, gradio("textbox"), gradio("Chat input"), show_progress=False)
                .then(
            ...
            shared.gradio["Replace last reply"].click(
                ...
                chat.replace_last_reply, gradio("textbox", "interface_state"), gradio("history")
            ).then(
                lambda: "", None, gradio("textbox"), show_progress=False
            ).then(
                chat.redraw_html, shared.reload_inputs, gradio("display")
            ).then(
                chat.save_persistent_history, gradio("history", "character_menu", "mode"), None
            ...
            shared.gradio["Send dummy message"].click(
                ...
                chat.send_dummy_message, gradio("textbox", "interface_state"), gradio("history")
            ).then(
                lambda: "", None, gradio("textbox"), show_progress=False
            ).then(
                chat.redraw_html, shared.reload_inputs, gradio("display")
            ...
            shared.gradio["Send dummy reply"].click(
                ...
                chat.send_dummy_reply, gradio("textbox", "interface_state"), gradio("history")
            ).then(
                lambda: "", None, gradio("textbox"), show_progress=False
            ).then(
                chat.redraw_html, shared.reload_inputs, gradio("display")
            ...
            shared.gradio["character_menu"].change(
                ...
                gradio("name1", "name2", "character_picture", "greeting", "context", "dummy"),
            ).then(
                ui.gather_interface_values, gradio(shared.input_elements), gradio("interface_state")
            ).then(
                chat.load_persistent_history, gradio("interface_state"), gradio("history")
            ).then(
                chat.redraw_html, shared.reload_inputs, gradio("display")
            )
            ...
            shared.gradio["upload_chat_history"].upload(
                chat.load_history, gradio("upload_chat_history", "history"), gradio("history")
            ).then(chat.redraw_html, shared.reload_inputs, gradio("display"))
            ...
            ).then(lambda: "characters/instruction-following/", None, gradio("save_root")).then(
                chat.generate_instruction_template_yaml,
                gradio("name1_instruct", "name2_instruct", "context_instruct", "turn_template"),
                gradio("save_contents"),
            ).then(
                lambda: gr.update(visible=True), None, gradio("file_saver")
            )
            ...
            shared.gradio["delete_template"].click(
                lambda x: f"{x}.yaml", gradio("instruction_template"), gradio("delete_filename")
            ).then(lambda: "characters/instruction-following/", None, gradio("delete_root")).then(
                lambda: gr.update(visible=True), None, gradio("file_deleter")
            )

            shared.gradio["download_button"].click(
                chat.save_history_at_user_request,
            ...
            shared.gradio["upload_img_tavern"].clear(
            ...
            shared.gradio["your_picture"].change(
                chat.upload_your_profile_picture, gradio("your_picture"), None
            ).then(
        ...
        # notebook/default modes event handlers
            ...
            gen_events.append(
                ...
                .then(
                    generate_reply_wrapper, shared.input_params, output_params, show_progress=False
                )
                .then(
                    ui.gather_interface_values,
                    gradio(shared.input_elements),
            ...
            if shared.args.notebook:
                shared.gradio["Undo"].click(
                    lambda x: x, gradio("last_input"), gradio("textbox"), show_progress=False
                )
                ...
                shared.gradio["markdown_render"].click(
                    lambda x: x, gradio("output_textbox"), gradio("markdown"), queue=False
                )
                gen_events.append(
                    shared.gradio["Continue"]
                    .click(
            ...
            shared.gradio["prompt_menu"].change(
                load_prompt, gradio("prompt_menu"), gradio("textbox"), show_progress=False
            )
            ...
            ).then(lambda x: x + ".txt", gradio("prompt_menu"), gradio("delete_filename")).then(
                lambda: gr.update(visible=True), None, gradio("file_deleter")
            )
