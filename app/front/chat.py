...
from app.engine.textgen import generate_reply, get_encoded_length, get_max_prompt_length
...
def get_turn_substrings(state, instruct=False):
    ...
    output = {
        ...
        "bot_turn_stripped": "<|bot|>"
        + template.split("<|bot|>")[1].split("<|bot-message|>")[0],
    }
...
def generate_chat_prompt(user_input, state, **kwargs):
    ...
    if state["mode"] == "chat-instruct":
        ...
        if impersonate:
            ...
            wrapper += apply_extensions(
                "bot_prefix", substrings["bot_turn_stripped"], state
            )
            wrapper += history[-1][1]
        else:
            wrapper += apply_extensions(
                "bot_prefix", substrings["bot_turn_stripped"].rstrip(" "), state
            )
    else:
        wrapper = "<|prompt|>"
    ...
    i = len(history) - 1
    rows = [
        state["context_instruct"] if is_instruct else f"{state['context'].strip()}\n"
    ]
    while (
        i >= 0
        and get_encoded_length(wrapper.replace("<|prompt|>", "".join(rows)))
        < max_length
    ):
        if _continue and i == len(history) - 1:
            ...
            rows.insert(
                1,
                substrings["bot_turn"].replace(
                    "<|bot-message|>", history[i][1].strip()
                ),
            )
        ...
        if string not in ["", "<|BEGIN-VISIBLE-CHAT|>"]:
            rows.insert(
                ...
                replace_all(
                    ...
                    {"<|user-message|>": string.strip(), "<|round|>": str(i)},
                ),
            )
...
def get_stopping_strings(state):
    ...
    if state["mode"] in ["instruct", "chat-instruct"]:
        ...
        for i in range(len(stopping_strings)):
            stopping_strings[i] = (
                replace_all(stopping_strings[i], replacements)
                .rstrip(" ")
                .replace(r"\n", "\n")
            )
...
def chatbot_wrapper(
    text, state, regenerate=False, _continue=False, loading_message=True
):
    ...
    if not any((regenerate, _continue)):
        ...
        if loading_message:
            yield {
                ...
                + [[visible_text, shared.processing_message]],
    ...
    for i in range(state["chat_generation_attempts"]):
        ...
        for j, reply in enumerate(
            ...
            if shared.stop_everything:
                output["visible"][-1][1] = apply_extensions(
                    "output", output["visible"][-1][1], state
                )
                yield output
                return
...
def generate_chat_reply(
    ...
    for history in chatbot_wrapper(
        text,
        state,
...
def generate_chat_reply_wrapper(
    text, start_with, state, regenerate=False, _continue=False
):
    ...
    for i, history in enumerate(
        generate_chat_reply(text, state, regenerate, _continue, loading_message=True)
    ):
        yield chat_html_wrapper(
            history, state["name1"], state["name2"], state["mode"], state["chat_style"]
        ), history


def remove_last_message(history):
    if (
        len(history["visible"]) > 0
...
def redraw_html(history, name1, name2, mode, style, reset_cache=False):
    return chat_html_wrapper(
        history, name1, name2, mode, style, reset_cache=reset_cache
    )
