    ...
    if shared.args.flexgen:
        ...
        return sorted(
            [
                ...
                for item in list(Path(f"{shared.args.model_dir}/").glob("*"))
                if not item.name.endswith((".txt", "-np", ".pt", ".json", ".yaml"))
            ],
            key=natural_keys,
        )
...
def get_available_extensions():
    return sorted(
        set(map(lambda x: x.parts[1], Path("extensions").glob("*/script.py"))),
        key=natural_keys,
    )
...
def get_available_loras():
    return sorted(
        [
            item.name
            for item in list(Path(shared.args.lora_dir).glob("*"))
            if not item.name.endswith((".txt", "-np", ".pt", ".json"))
        ],
        key=natural_keys,
    )
...
def get_available_sessions():
    ...
    return [item for item in items if "autosave" in item] + [
        item for item in items if "autosave" not in item
    ]
