...
def encode(prompt, add_special_tokens=True, add_bos_token=True, truncation_length=None):
    ...
    if (
        ...
        in ["LlamaCppModel", "RWKVModel", "ExllamaModel"]
        or shared.args.cpu
    ):
        return input_ids
...
def get_reply_from_output_ids(
    output_ids, input_ids, original_question, state, is_chat=False
):
    if shared.is_seq2seq:
        ...
        if (
            type(shared.tokenizer)
            in [transformers.LlamaTokenizer, transformers.LlamaTokenizerFast]
            and len(output_ids) > 0
        ):
            if shared.tokenizer.convert_ids_to_tokens(
...
def _generate_reply(question, state, stopping_strings=None, is_chat=False):
    ...
    for reply in generate_func(
        ...
        if is_stream:
            cur_time = time.time()
            if (
                cur_time - last_update > 0.041666666666666664
            ):  # Limit streaming to 24 fps
    ...
    try:
        ...
        # Stream the output naively for FlexGen since it doesn't support 'stopping_criteria'
            ...
            for i in range(state["max_new_tokens"] // 8 + 1):
                ...
                if np.count_nonzero(
                    np.isin(input_ids[0], eos_token_ids)
                ) < np.count_nonzero(np.isin(output, eos_token_ids)):
                    break

                yield get_reply_from_output_ids(
                    output, original_input_ids, original_question, state
                )
