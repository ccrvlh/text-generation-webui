...
def create_train_interface():
    with gr.Tab("Train LoRA", elem_id="lora-train-tab"):
        gr.Markdown(
            "Confused? [[Click here for a guide]](https://github.com/oobabooga/text-generation-webui/blob/main/docs/Training-LoRAs.md)"
        )
        ...
        with gr.Row():
            ...
            always_override = gr.Checkbox(
                ...
                value=False,
                info="If the name given is the same as an existing file, checking this will replace that file. Leaving unchecked will load that file and continue from it (must use the same rank value as the original had).",
            )
            save_steps = gr.Number(
                label="Save every n steps",
                value=0,
                info="If above 0, a checkpoint of the LoRA will be saved every time this many steps pass.",
            )
        ...
        with gr.Row():
            ...
            micro_batch_size = gr.Slider(
                ...
                maximum=128,
                step=1,
                info="Per-device batch size (NOTE: multiple devices not yet implemented). Increasing this will increase VRAM usage.",
            )
            batch_size = gr.Slider(
                ...
                maximum=1024,
                step=4,
                info="Global batch size. The two batch sizes together determine gradient accumulation (gradientAccum = batch / microBatch). Higher gradient accum values lead to better quality training.",
            )
        ...
        with gr.Row():
            epochs = gr.Number(
                label="Epochs",
                value=3,
                info="Number of times every entry in the dataset should be fed into training. So 1 means feed each item in once, 5 means feed it in five times, etc.",
            )
            learning_rate = gr.Textbox(
                label="Learning Rate",
                value="3e-4",
                info="Learning rate, in scientific notation. 3e-4 is a good starting base point. 1e-2 is extremely high, 1e-6 is extremely low.",
            )
            lr_scheduler_type = gr.Dropdown(
                ...
                info='Learning rate scheduler - defines how the learning rate changes over time. "Constant" means never change, "linear" means to go in a straight line from the learning rate down to 0, cosine follows a curve, etc.',
            )
        ...
        lora_alpha = gr.Slider(
            ...
            maximum=2048,
            step=4,
            info="LoRA Alpha. This divided by the rank becomes the scaling of the LoRA. Higher means stronger. A good standard value is twice your Rank.",
        )
        ...
        cutoff_len = gr.Slider(
            ...
            value=256,
            step=32,
            info="Cutoff length for text input. Essentially, how long of a line of text to feed in at a time. Higher values require drastically more VRAM.",
        )
        ...
        with gr.Tab(label="Formatted Dataset"):
            with gr.Row():
                dataset = gr.Dropdown(
                    ...
                    value="None",
                    label="Dataset",
                    info="The dataset file to use for training.",
                )
                ui.create_refresh_button(
                    dataset,
                    lambda: None,
                ...
                eval_dataset = gr.Dropdown(
                    ...
                    info="The (optional) dataset file used to evaluate the model after training.",
                )
                ui.create_refresh_button(
                    eval_dataset,
                    lambda: None,
                ...
                format = gr.Dropdown(
                    ...
                    value="None",
                    label="Data Format",
                    info="The format file used to decide how to format the dataset input.",
                )
                ui.create_refresh_button(
                    format,
                    lambda: None,
            ...
            eval_steps = gr.Number(
                ...
                value=100,
                info="If an evaluation dataset is given, test it every time this many steps pass.",
            )
        ...
        with gr.Tab(label="Raw text file"):
            with gr.Row():
                raw_text_file = gr.Dropdown(
                    ...
                    value="None",
                    label="Text file",
                    info="The raw text file to use for training.",
                )
                ui.create_refresh_button(
                    raw_text_file,
                    lambda: None,
                ...
                min_chars = gr.Number(
                    label="Ignore small blocks",
                    value=0,
                    info="Ignore Hard Cut blocks that have less or equal characters than this number",
                )
            ...
            with gr.Row():
                overlap_len = gr.Slider(
                    ...
                    value=128,
                    step=16,
                    info="Overlap length - ie how many tokens from the prior chunk of text to include into the next chunk. (The chunks themselves will be of a size determined by Cutoff Length below). Setting overlap to exactly half the cutoff length may be ideal.",
                )
                newline_favor_len = gr.Slider(
                    ...
                    value=128,
                    step=16,
                    info="Length (in characters, not tokens) of the maximum distance to shift an overlap cut by to ensure chunks cut at newlines. If too low, cuts may occur in the middle of lines.",
                )
        ...
        with gr.Accordion(label="Advanced Options", open=False):
            lora_dropout = gr.Slider(
                ...
                step=0.025,
                value=0.05,
                info="Percentage probability for dropout of LoRA layers. This can help reduce overfitting. Most users should leave at default.",
            )
            warmup_steps = gr.Number(
                label="Warmup Steps",
                value=100,
                info="For this many steps at the start, the learning rate will be lower than normal. This helps the trainer prepare the model and precompute statistics to improve the quality of training after the start.",
            )
            ...
            stop_at_loss = gr.Slider(
                label="Stop at loss",
                minimum=0.0,
                maximum=3.0,
                step=0.1,
                value=0.00,
                info="The process will automatically stop once the desired loss value is reached. (reasonable numbers are 1.5-1.8)",
            )
            add_eos_token = gr.Checkbox(
                label="Add EOS token",
                value=False,
                info="Adds EOS token for each dataset item. In case of raw text, the EOS will be added at the Hard Cut",
            )
            ...
            with gr.Row():
                higher_rank_limit = gr.Checkbox(
                    ...
                    value=False,
                    info="If checked, changes Rank/Alpha slider above to go much higher. This will not work without a datacenter-class GPU.",
                )
            with gr.Row():
                report_to = gr.Radio(
                    label="Save detailed logs with",
                    value="None",
            ...
            with gr.Column():
                ...
                with gr.Row():
                    stride_length = gr.Slider(
                        ...
                        value=512,
                        step=1,
                        info="Used to make the evaluation faster at the cost of accuracy. 1 = slowest but most accurate. 512 is a common value.",
                    )
                    max_length = gr.Slider(
                        ...
                        value=0,
                        step=1,
                        info="The context for each evaluation. If set to 0, the maximum context length for the model will be used.",
                    )
...
def do_train(
    ...
    if model_type in MODEL_CLASSES:
        ...
        if model_type == "PeftModelForCausalLM":
            if len(shared.lora_names) > 0:
                ...
                logger.warning(
                    "Training LoRA over top of another LoRA. May have unexpected effects."
                )
            else:
                ...
                logger.warning(
                    "Model ID not matched due to LoRA loading. Consider reloading base model."
                )
        else:
            ...
            logger.warning(
                f"LoRA training has only currently been validated for LLaMA, OPT, GPT-J, and GPT-NeoX models. (Found model type: {model_type})"
            )
    ...
    def tokenize(prompt, append_eos_token=False):
        ...
        if train_only_after == "" or train_only_after not in prompt:
            ...
            if (
                append_eos_token
                and input_ids[-1] != shared.tokenizer.eos_token_id
    ...
    if shared.model_dirty_from_training:
        ...
        if selected_model:
            print(
                "\033[1;31;1m(Model has been modified by previous training, it needs to be reloaded...)\033[0;37;0m"
            )
            try:
        ...
        def on_step_begin(
            ...
            **kwargs,
            ...
            if WANT_INTERRUPT:
                ...
                with open(
                    f"{lora_file_path}/checkpoint-{tracked.current_steps}/training_prompt.json",
                    "w",
                    encoding="utf-8",
                ) as file:
    ...
    if lora_all_param > 0:
        print(
            f"Trainable params: {lora_trainable_param:,d} ({100 * lora_trainable_param / lora_all_param:.4f} %), All params: {lora_all_param:,d} (Model: {model_all_params:,d})"
        )
